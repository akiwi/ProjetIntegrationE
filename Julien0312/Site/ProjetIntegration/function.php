<?php
	...
	function updateUser($db,$id,$name,$mdp,$droit){
		...
		if($find > 0 ){
			...
			if($row['idClient'] != $id){
				...
				if($mdp != ""){
					$result = updatePass($db,$id,$name,$mdp,$droit);
				}
				else{
					$result = updatePassEmpty($db,$id,$name,$droit);			
				}
			}	
		}
		else{
	...
	function updatePassEmpty($db,$id,$name,$droit){
		
		$updateUser = "UPDATE CLIENT SET nameUser='".$name."', droit ='".$droit."' WHERE idClient ='".$id."';";
		$result = $db->query($updateUser);

		return $result;
	}

	function updatePass($db,$id,$name,$mdp,$droit){
		$updateUser = "UPDATE CLIENT SET nameUser='".$name."', pwdUser = '".$mdp."', droit ='".$droit."' WHERE idClient ='".$id."';";
		$result = $db->query($updateUser);
